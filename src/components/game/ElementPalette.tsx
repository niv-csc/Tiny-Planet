...
import { TreePine, Factory, Waves, Mountain, User, FlaskConical, GraduationCap, Hammer, Heart } from 'lucide-react';
...
const ElementPalette = () => {
  ...
  const actionsRemaining = maxActionsPerGeneration - actionsThisGeneration;
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-1">
            {Array.from({ length: maxActionsPerGeneration }).map((_, i) => (
              <motion.div
                key={i}
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-9 gap-2">
        {elements.map((element, index) => (
          <motion.button
            ...
            `}
          >
            <div className={`${categoryTextColors[element.category]}`}>
              {element.icon}
            </div>
